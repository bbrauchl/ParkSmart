...
def select_region( img ):
    ...
    pnt_1 = [ cols * 0.30, rows * 0.05 ] # TL
    pnt_2 = [ cols * 0.88, rows * 0.05 ] # TR
    pnt_3 = [ cols * 0.88, rows * 0.90 ] # BR
...
def camera_pipeline( img ):
    ...
    img = select_region( img ) # Get cropped img

    return( img )
#-------------------------------------------------------------------------------
"""
Pipeline for camera feed with jetson nano
"""
...
def show_camera():
    ...
    display_width=3264//8 #1280,
    display_height=2464//8 #720,
    if camera.isOpened():
        ...
        while( cv.getWindowProperty("CSI Camera", 0) >= 0 ):
            ...
            if( ret == True ):
                frame = camera_pipeline(frame)
                cv.resizeWindow( "CSI Camera", (display_width, display_height ) )
